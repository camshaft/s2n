...
int s2n_tls13_keys_init(struct s2n_tls13_keys *keys, s2n_hmac_algorithm alg)
{
    ...
    GUARD_AS_POSIX(s2n_blob_init(&keys->extract_secret, keys->extract_secret_bytes, keys->size));
    GUARD_AS_POSIX(s2n_blob_init(&keys->derive_secret, keys->derive_secret_bytes, keys->size));
