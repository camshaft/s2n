...
S2N_RESULT s2n_get_public_random_bytes_used(void)
{
    GUARD_AS_RESULT(s2n_drbg_bytes_used(&per_thread_public_drbg));
    return S2N_RESULT_OK;
}

S2N_RESULT s2n_get_private_random_bytes_used(void)
{
    GUARD_AS_RESULT(s2n_drbg_bytes_used(&per_thread_private_drbg));
    return S2N_RESULT_OK;
}
 ...
 */
S2N_RESULT s2n_public_random(int64_t bound, int64_t *output)
{
    uint64_t r;

    gt_check_result(bound, 0);
...
S2N_RESULT s2n_rand_init(void)
{
  OPEN:
    ...
    if (entropy_fd == -1) {
        ...
        S2N_ERROR_RESULT(S2N_ERR_OPEN_RANDOM);
    }
#if defined(MAP_INHERIT_ZERO)
    zero_if_forked_ptr = mmap(NULL, sizeof(int), PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE, -1, 0);
    S2N_ERROR_RESULT_IF(zero_if_forked_ptr == MAP_FAILED, S2N_ERR_OPEN_RANDOM);

    S2N_ERROR_RESULT_IF(minherit(zero_if_forked_ptr, sizeof(int), MAP_INHERIT_ZERO) == -1, S2N_ERR_OPEN_RANDOM);
#else

    S2N_ERROR_RESULT_IF(pthread_atfork(NULL, NULL, s2n_on_fork) != 0, S2N_ERR_OPEN_RANDOM);
#endif
    ...
    S2N_ERROR_RESULT_IF(e == NULL || ENGINE_init(e) != 1 || ENGINE_set_default(e, ENGINE_METHOD_RAND) != 1 || ENGINE_free(e) != 1, S2N_ERR_OPEN_RANDOM);
#endif
...
{
    S2N_ERROR_RESULT_IF(entropy_fd == -1, S2N_ERR_NOT_INITIALIZED);
...
{
    S2N_ERROR_RESULT_IF(!s2n_in_unit_test(), S2N_ERR_NOT_IN_UNIT_TEST);
    GUARD_AS_RESULT(s2n_drbg_wipe(&per_thread_private_drbg));
...
S2N_RESULT s2n_cpu_supports_rdrand()
{
#if ((defined(__x86_64__) || defined(__i386__)) && (defined(__clang__) || S2N_GCC_VERSION_AT_LEAST(4,3,0)))
...
S2N_RESULT s2n_get_rdrand_data(struct s2n_blob *out)
{
    ...
    while ((space_remaining = s2n_stuffer_space_remaining(&stuffer))) {
        ...
        S2N_ERROR_RESULT_IF(!success, S2N_ERR_RDRAND_FAILED);
    ...
    return S2N_RESULT_OK;
#else
    S2N_ERROR_RESULT(S2N_ERR_UNSUPPORTED_CPU);
#endif
}
