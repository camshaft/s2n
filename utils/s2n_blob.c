...
{
    CHECKED_MEMSET(b->data, 0, b->size);
...
{
    S2N_ERROR_IF_NULL(b);
    S2N_ERROR_IF_NULL(slice);

    S2N_ERROR_RESULT_IF(b->size < (offset + size), S2N_ERR_SIZE_MISMATCH);
...
S2N_RESULT s2n_hex_string_to_bytes(const char *str, struct s2n_blob *blob)
{
    S2N_ERROR_IF_NULL(str);
    S2N_ERROR_IF_NULL(blob);
    uint32_t len = strlen(str);
    /* protects against overflows */
    gte_check_result(blob->size, len / 2);
    S2N_ERROR_RESULT_IF(len % 2 != 0, S2N_ERR_INVALID_HEX);

    for (int i = 0; i < len; i += 2) {
        uint8_t high_nibble = hex_inverse[(uint8_t) str[i]];
        S2N_ERROR_RESULT_IF(high_nibble == 255, S2N_ERR_INVALID_HEX);

        uint8_t low_nibble = hex_inverse[(uint8_t) str[i + 1]];
        S2N_ERROR_RESULT_IF(low_nibble == 255, S2N_ERR_INVALID_HEX);
