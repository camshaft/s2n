...
S2N_RESULT s2n_inet_ntop(int af, const void *addr, struct s2n_blob *dst)
{
    ...
    if (af == AF_INET) {
        S2N_ERROR_RESULT_IF(dst->size < sizeof("111.222.333.444"), S2N_ERR_SIZE_MISMATCH);
    ...
    if (af == AF_INET6) {
        S2N_ERROR_RESULT_IF(dst->size < sizeof("1111:2222:3333:4444:5555:6666:7777:8888"), S2N_ERR_SIZE_MISMATCH);
    ...
    S2N_ERROR_RESULT(S2N_ERR_INVALID_ARGUMENT);
}
